...
import { s3 } from '../../../config/r2.config';
...
const parseS3Url = (s3Url: string): { bucket: string; key: string } => {
  ...
  // Legacy AWS virtual-hosted-style URLs (bucket.s3.region.amazonaws.com)
  // carry the bucket name in the host. R2 public/custom-domain URLs
  // (pub-xxx.r2.dev, cdn.example.com) don't encode the bucket in the host
  // at all, so fall back to the configured R2 bucket for those.
  if (hostParts[1] === 's3') {
    return {
      bucket: hostParts[0],
  ...
  return {
    bucket: config.r2.bucketName as string,
    key: decodeURIComponent(url.pathname.substring(1)),
  };
};
