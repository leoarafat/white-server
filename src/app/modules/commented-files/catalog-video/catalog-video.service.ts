...
//     // 2. Get video with populated data
//     logger.info(`${transactionId} Fetching video data`);
//     const video = await Video.findById(id)
//       .populate('label')
//       .populate('user')
//       .populate('primaryArtist');
...
//     // 3. Ensure duration exists (create if missing)
//     if (!video.durationMs) {
...
//     // 4. Generate DDEX XML (only after duration is confirmed)
...
//       //@ts-ignore
//       throw new ApiError(httpStatus.INTERNAL_SERVER_ERROR, xmlError);
//     }

//     // 5. Upload all files to VEVO
...
//     // 6. Mark as VEVO (only after successful XML generation and file uploads)
...
//       throw new ApiError(
//         httpStatus.INTERNAL_SERVER_ERROR,
//         'Video processed but status update failed',
//       );
//     }
//   } catch (error) {
