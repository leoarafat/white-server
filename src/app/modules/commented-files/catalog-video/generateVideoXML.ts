...
// import { getFileMd5FromS3 } from './utils';
...
//     const videoHash = await getFileMd5FromS3(video.video);
//     const imageHash = await getFileMd5FromS3(video.image);
...
//       .ele('FullName')
//       .txt(video.primaryArtist[0]) // Assuming primaryArtist is string[]
//       .up()
//       .up();
//     party1.up();

//     // Second Main Artist (P2) - Only if exists
...
//     // Featured Artist (P3) - Only if exists
...
//     // ========== Party References (Composers, Directors, etc.) ==========
...
//     // Display Artists (Only add those that exist)
//     videoResource
//       .ele('DisplayArtistName', { IsDefault: 'true' })
//       .txt(video.primaryArtist.join(', ')) // Join all primary artists
//       .up();
...
//     // Featured artist (only if exists)
//     if (video.featuringArtists && video.featuringArtists.length > 0) {
...
//     // Contributors (only add those that exist)
...
//     // Second contributor (only if exists)
//     if (video.primaryArtist.length > 1) {
...
//     // Featured contributor (only if exists)
//     if (video.featuringArtists && video.featuringArtists.length > 0) {
...
//     // Display Artists (only those that exist)
//     mainRelease
//       .ele('DisplayArtistName', { IsDefault: 'true' })
//       .txt(video.primaryArtist.join(', '))
//       .up();
